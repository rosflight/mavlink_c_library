...
{
 float aux_cmd_array[8]; /*< */
 uint8_t type_array; /*< */
} mavlink_rosflight_aux_cmd_t;

#define MAVLINK_MSG_ID_ROSFLIGHT_AUX_CMD_LEN 33
#define MAVLINK_MSG_ID_193_LEN 33

#define MAVLINK_MSG_ID_ROSFLIGHT_AUX_CMD_CRC 51
#define MAVLINK_MSG_ID_193_CRC 51

#define MAVLINK_MSG_ROSFLIGHT_AUX_CMD_FIELD_AUX_CMD_ARRAY_LEN 8

#define MAVLINK_MESSAGE_INFO_ROSFLIGHT_AUX_CMD { \
	"ROSFLIGHT_AUX_CMD", \
	2, \
	{  { "aux_cmd_array", NULL, MAVLINK_TYPE_FLOAT, 8, 0, offsetof(mavlink_rosflight_aux_cmd_t, aux_cmd_array) }, \
         { "type_array", NULL, MAVLINK_TYPE_UINT8_T, 0, 32, offsetof(mavlink_rosflight_aux_cmd_t, type_array) }, \
         } \
}
...
static inline uint16_t mavlink_msg_rosflight_aux_cmd_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
						       uint8_t type_array, const float *aux_cmd_array)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[MAVLINK_MSG_ID_ROSFLIGHT_AUX_CMD_LEN];
	_mav_put_uint8_t(buf, 32, type_array);
	_mav_put_float_array(buf, 0, aux_cmd_array, 8);
        memcpy(_MAV_PAYLOAD_NON_CONST(msg), buf, MAVLINK_MSG_ID_ROSFLIGHT_AUX_CMD_LEN);
#else
	mavlink_rosflight_aux_cmd_t packet;
	packet.type_array = type_array;
	mav_array_memcpy(packet.aux_cmd_array, aux_cmd_array, sizeof(float)*8);
...
static inline void mavlink_msg_rosflight_aux_cmd_send(mavlink_channel_t chan, uint8_t type_array, const float *aux_cmd_array)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[MAVLINK_MSG_ID_ROSFLIGHT_AUX_CMD_LEN];
	_mav_put_uint8_t(buf, 32, type_array);
 ...
 */
static inline void mavlink_msg_rosflight_aux_cmd_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  uint8_t type_array, const float *aux_cmd_array)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
 ...
 * @return 
 */
static inline uint8_t mavlink_msg_rosflight_aux_cmd_get_type_array(const mavlink_message_t* msg)
{
...
{
	return _MAV_RETURN_float_array(msg, aux_cmd_array, 8,  0);
}
...
static inline void mavlink_msg_rosflight_aux_cmd_decode(const mavlink_message_t* msg, mavlink_rosflight_aux_cmd_t* rosflight_aux_cmd)
{
	...
	rosflight_aux_cmd->type_array = mavlink_msg_rosflight_aux_cmd_get_type_array(msg);
#else
