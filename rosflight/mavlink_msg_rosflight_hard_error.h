...
#define MAVLINK_MSG_ID_ROSFLIGHT_HARD_ERROR_LEN 12
#define MAVLINK_MSG_ID_198_LEN 12

#define MAVLINK_MSG_ID_ROSFLIGHT_HARD_ERROR_CRC 106
#define MAVLINK_MSG_ID_198_CRC 106
...
#define MAVLINK_MESSAGE_INFO_ROSFLIGHT_HARD_ERROR { \
	...
	3, \
	{  { "pc", NULL, MAVLINK_TYPE_UINT32_T, 0, 0, offsetof(mavlink_rosflight_hard_error_t, pc) }, \
         { "reset_count", NULL, MAVLINK_TYPE_UINT32_T, 0, 4, offsetof(mavlink_rosflight_hard_error_t, reset_count) }, \
         { "doRearm", NULL, MAVLINK_TYPE_UINT32_T, 0, 8, offsetof(mavlink_rosflight_hard_error_t, doRearm) }, \
         } \
}
...
static inline uint16_t mavlink_msg_rosflight_hard_error_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
						       uint32_t pc, uint32_t reset_count, uint32_t doRearm)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[MAVLINK_MSG_ID_ROSFLIGHT_HARD_ERROR_LEN];
	_mav_put_uint32_t(buf, 0, pc);
	_mav_put_uint32_t(buf, 4, reset_count);
	_mav_put_uint32_t(buf, 8, doRearm);
...
{
	return mavlink_msg_rosflight_hard_error_pack(system_id, component_id, msg, rosflight_hard_error->pc, rosflight_hard_error->reset_count, rosflight_hard_error->doRearm);
}
...
{
	return mavlink_msg_rosflight_hard_error_pack_chan(system_id, component_id, chan, msg, rosflight_hard_error->pc, rosflight_hard_error->reset_count, rosflight_hard_error->doRearm);
}
...
static inline void mavlink_msg_rosflight_hard_error_send(mavlink_channel_t chan, uint32_t pc, uint32_t reset_count, uint32_t doRearm)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[MAVLINK_MSG_ID_ROSFLIGHT_HARD_ERROR_LEN];
	_mav_put_uint32_t(buf, 0, pc);
 ...
 */
static inline void mavlink_msg_rosflight_hard_error_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  uint32_t pc, uint32_t reset_count, uint32_t doRearm)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
