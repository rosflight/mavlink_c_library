...
#define MAVLINK_MSG_ID_ROSFLIGHT_HARD_ERROR_LEN 40
#define MAVLINK_MSG_ID_198_LEN 40

#define MAVLINK_MSG_ID_ROSFLIGHT_HARD_ERROR_CRC 244
#define MAVLINK_MSG_ID_198_CRC 244
...
#define MAVLINK_MESSAGE_INFO_ROSFLIGHT_HARD_ERROR { \
	...
	{  { "r0", NULL, MAVLINK_TYPE_UINT32_T, 0, 0, offsetof(mavlink_rosflight_hard_error_t, r0) }, \
         ...
         { "reset_count", NULL, MAVLINK_TYPE_UINT32_T, 0, 32, offsetof(mavlink_rosflight_hard_error_t, reset_count) }, \
         { "doRearm", NULL, MAVLINK_TYPE_UINT32_T, 0, 36, offsetof(mavlink_rosflight_hard_error_t, doRearm) }, \
         } \
}
...
static inline uint16_t mavlink_msg_rosflight_hard_error_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
						       uint32_t r0, uint32_t r1, uint32_t r2, uint32_t r3, uint32_t r12, uint32_t lr, uint32_t pc, uint32_t psr, uint32_t reset_count, uint32_t doRearm)
{
	...
	_mav_put_uint32_t(buf, 32, reset_count);
	_mav_put_uint32_t(buf, 36, doRearm);
...
{
	return mavlink_msg_rosflight_hard_error_pack(system_id, component_id, msg, rosflight_hard_error->r0, rosflight_hard_error->r1, rosflight_hard_error->r2, rosflight_hard_error->r3, rosflight_hard_error->r12, rosflight_hard_error->lr, rosflight_hard_error->pc, rosflight_hard_error->psr, rosflight_hard_error->reset_count, rosflight_hard_error->doRearm);
}
...
{
	return mavlink_msg_rosflight_hard_error_pack_chan(system_id, component_id, chan, msg, rosflight_hard_error->r0, rosflight_hard_error->r1, rosflight_hard_error->r2, rosflight_hard_error->r3, rosflight_hard_error->r12, rosflight_hard_error->lr, rosflight_hard_error->pc, rosflight_hard_error->psr, rosflight_hard_error->reset_count, rosflight_hard_error->doRearm);
}
...
static inline void mavlink_msg_rosflight_hard_error_send(mavlink_channel_t chan, uint32_t r0, uint32_t r1, uint32_t r2, uint32_t r3, uint32_t r12, uint32_t lr, uint32_t pc, uint32_t psr, uint32_t reset_count, uint32_t doRearm)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[MAVLINK_MSG_ID_ROSFLIGHT_HARD_ERROR_LEN];
	_mav_put_uint32_t(buf, 0, r0);
 ...
 */
static inline void mavlink_msg_rosflight_hard_error_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  uint32_t r0, uint32_t r1, uint32_t r2, uint32_t r3, uint32_t r12, uint32_t lr, uint32_t pc, uint32_t psr, uint32_t reset_count, uint32_t doRearm)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
