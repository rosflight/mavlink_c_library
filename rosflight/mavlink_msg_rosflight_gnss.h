...
#define MAVLINK_MSG_ID_ROSFLIGHT_GNSS_CRC 192
#define MAVLINK_MSG_ID_197_CRC 192
...
#define MAVLINK_MESSAGE_INFO_ROSFLIGHT_GNSS { \
	...
	{  { "seconds", NULL, MAVLINK_TYPE_UINT64_T, 0, 0, offsetof(mavlink_rosflight_gnss_t, seconds) }, \
         ...
         { "nanos", NULL, MAVLINK_TYPE_UINT32_T, 0, 32, offsetof(mavlink_rosflight_gnss_t, nanos) }, \
...
static inline uint16_t mavlink_msg_rosflight_gnss_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
						       uint64_t seconds, uint32_t nanos, uint8_t fix_type, uint8_t num_sat, double lat, double lon, float height, float vel_n, float vel_e, float vel_d, float h_acc, float v_acc, float s_acc, uint64_t rosflight_timestamp)
{
	...
	_mav_put_uint32_t(buf, 32, nanos);
...
static inline void mavlink_msg_rosflight_gnss_send(mavlink_channel_t chan, uint64_t seconds, uint32_t nanos, uint8_t fix_type, uint8_t num_sat, double lat, double lon, float height, float vel_n, float vel_e, float vel_d, float h_acc, float v_acc, float s_acc, uint64_t rosflight_timestamp)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[MAVLINK_MSG_ID_ROSFLIGHT_GNSS_LEN];
	_mav_put_uint64_t(buf, 0, seconds);
 ...
 */
static inline void mavlink_msg_rosflight_gnss_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  uint64_t seconds, uint32_t nanos, uint8_t fix_type, uint8_t num_sat, double lat, double lon, float height, float vel_n, float vel_e, float vel_d, float h_acc, float v_acc, float s_acc, uint64_t rosflight_timestamp)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
