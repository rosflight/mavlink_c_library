...
#define MAVLINK_MSG_ID_ROSFLIGHT_INS_MIN_LEN 52
#define MAVLINK_MSG_ID_194_LEN 52
#define MAVLINK_MSG_ID_194_MIN_LEN 52
                               ...
                               float pos_north, float pos_east, float pos_down, float qw, float qx, float qy, float qz, float u, float v, float w, float p, float q, float r)
{
    ...
    return mavlink_finalize_message(msg, system_id, component_id, MAVLINK_MSG_ID_ROSFLIGHT_INS_MIN_LEN, MAVLINK_MSG_ID_ROSFLIGHT_INS_LEN, MAVLINK_MSG_ID_ROSFLIGHT_INS_CRC);
}
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_rosflight_ins_pack_status(uint8_t system_id, uint8_t component_id, mavlink_status_t *_status, mavlink_message_t* msg,
                               float pos_north, float pos_east, float pos_down, float qw, float qx, float qy, float qz, float u, float v, float w, float p, float q, float r)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_ROSFLIGHT_INS_LEN];
    ...
    return mavlink_finalize_message_buffer(msg, system_id, component_id, _status, MAVLINK_MSG_ID_ROSFLIGHT_INS_MIN_LEN, MAVLINK_MSG_ID_ROSFLIGHT_INS_LEN, MAVLINK_MSG_ID_ROSFLIGHT_INS_CRC);
#else
    return mavlink_finalize_message_buffer(msg, system_id, component_id, _status, MAVLINK_MSG_ID_ROSFLIGHT_INS_MIN_LEN, MAVLINK_MSG_ID_ROSFLIGHT_INS_LEN);
#endif
}
                                   ...
                                   float pos_north,float pos_east,float pos_down,float qw,float qx,float qy,float qz,float u,float v,float w,float p,float q,float r)
{
    ...
    msg->msgid = MAVLINK_MSG_ID_ROSFLIGHT_INS;
    return mavlink_finalize_message_chan(msg, system_id, component_id, chan, MAVLINK_MSG_ID_ROSFLIGHT_INS_MIN_LEN, MAVLINK_MSG_ID_ROSFLIGHT_INS_LEN, MAVLINK_MSG_ID_ROSFLIGHT_INS_CRC);
}
...
/**
 * @brief Encode a rosflight_ins struct with provided status structure
 *
 * @param system_id ID of this system
 * @param component_id ID of this component (e.g. 200 for IMU)
 * @param status MAVLink status structure
 ...
 * @param rosflight_ins C-struct to read the message contents from
 */
static inline uint16_t mavlink_msg_rosflight_ins_encode_status(uint8_t system_id, uint8_t component_id, mavlink_status_t* _status, mavlink_message_t* msg, const mavlink_rosflight_ins_t* rosflight_ins)
{
    return mavlink_msg_rosflight_ins_pack_status(system_id, component_id, _status, msg,  rosflight_ins->pos_north, rosflight_ins->pos_east, rosflight_ins->pos_down, rosflight_ins->qw, rosflight_ins->qx, rosflight_ins->qy, rosflight_ins->qz, rosflight_ins->u, rosflight_ins->v, rosflight_ins->w, rosflight_ins->p, rosflight_ins->q, rosflight_ins->r);
}
...
static inline void mavlink_msg_rosflight_ins_send(mavlink_channel_t chan, float pos_north, float pos_east, float pos_down, float qw, float qx, float qy, float qz, float u, float v, float w, float p, float q, float r)
{
    ...
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_ROSFLIGHT_INS, buf, MAVLINK_MSG_ID_ROSFLIGHT_INS_MIN_LEN, MAVLINK_MSG_ID_ROSFLIGHT_INS_LEN, MAVLINK_MSG_ID_ROSFLIGHT_INS_CRC);
#else
    mavlink_rosflight_ins_t packet;
    packet.pos_north = pos_north;
    packet.pos_east = pos_east;
    ...
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_ROSFLIGHT_INS, (const char *)&packet, MAVLINK_MSG_ID_ROSFLIGHT_INS_MIN_LEN, MAVLINK_MSG_ID_ROSFLIGHT_INS_LEN, MAVLINK_MSG_ID_ROSFLIGHT_INS_CRC);
#endif
}
 ...
 */
static inline void mavlink_msg_rosflight_ins_send_struct(mavlink_channel_t chan, const mavlink_rosflight_ins_t* rosflight_ins)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    mavlink_msg_rosflight_ins_send(chan, rosflight_ins->pos_north, rosflight_ins->pos_east, rosflight_ins->pos_down, rosflight_ins->qw, rosflight_ins->qx, rosflight_ins->qy, rosflight_ins->qz, rosflight_ins->u, rosflight_ins->v, rosflight_ins->w, rosflight_ins->p, rosflight_ins->q, rosflight_ins->r);
#else
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_ROSFLIGHT_INS, (const char *)rosflight_ins, MAVLINK_MSG_ID_ROSFLIGHT_INS_MIN_LEN, MAVLINK_MSG_ID_ROSFLIGHT_INS_LEN, MAVLINK_MSG_ID_ROSFLIGHT_INS_CRC);
#endif
}
...
#else
        uint8_t len = msg->len < MAVLINK_MSG_ID_ROSFLIGHT_INS_LEN? msg->len : MAVLINK_MSG_ID_ROSFLIGHT_INS_LEN;
        memset(rosflight_ins, 0, MAVLINK_MSG_ID_ROSFLIGHT_INS_LEN);
    memcpy(rosflight_ins, _MAV_PAYLOAD(msg), len);
#endif
}
